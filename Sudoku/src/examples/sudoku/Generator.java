...
import static examples.sudoku.Constants.STRANA_CTVERCE;
import static examples.sudoku.Constants.CTVEREC_PRVKU;
...
public class Generator {
    ...
    public static int[][] generateSolved() {
        int[][] array = new int[CTVEREC_PRVKU][CTVEREC_PRVKU];
        for (int i = 0; i < CTVEREC_PRVKU; i++) {
            for (int j = 0; j < CTVEREC_PRVKU; j++) {
                array[i][j] = (i * STRANA_CTVERCE + i / STRANA_CTVERCE + j) % CTVEREC_PRVKU + 1;
            }
        }
    ...
    private static void shuffleSquareRows(int[][] array) {
        Random random = new Random();
        for (int i = 0; i < STRANA_CTVERCE - 1; i++) {
            int j = 1 + i + random.nextInt(STRANA_CTVERCE - 1 - i);
    ...
    private static void shuffleSingleRows(int[][] array) {
        ...
        for (int i = 0; i < STRANA_CTVERCE; i++) {
            int start = i * STRANA_CTVERCE;
            int limit = start + STRANA_CTVERCE - 1;
    ...
    private static void swapSingleRows(int[][] array, int i, int j) {
        int[] temp = new int[CTVEREC_PRVKU];
        System.arraycopy(array[i], 0, temp, 0, CTVEREC_PRVKU);
        System.arraycopy(array[j], 0, array[i], 0, CTVEREC_PRVKU);
        System.arraycopy(temp, 0, array[j], 0, CTVEREC_PRVKU);
    }
    ...
    private static void swapSquareRows(int[][] array, int i, int j) {
        //if (i == j) return;
        int[][] temp = new int[STRANA_CTVERCE][CTVEREC_PRVKU];
        int iStart = i * STRANA_CTVERCE;
        int jStart = j * STRANA_CTVERCE;
        int iLimit = iStart + STRANA_CTVERCE;
        int jLimit = jStart + STRANA_CTVERCE;
        // copy to temp
        for (int k = iStart, l = 0; k < iLimit; k++, l++) {
            System.arraycopy(array[k], 0, temp[l], 0, CTVEREC_PRVKU);
        }
        // copy to array[i] & following
        for (int k = iStart, l = jStart; k < iLimit; k++, l++) {
            System.arraycopy(array[l], 0, array[k], 0, CTVEREC_PRVKU);
        }
        // copy to array[j] & following
        for (int k = jStart, l = 0; k < jLimit; k++, l++) {
            System.arraycopy(temp[l], 0, array[k], 0, CTVEREC_PRVKU);
        }
    }
